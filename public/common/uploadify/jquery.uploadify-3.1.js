		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					alert('该文件名为 "' + file.name + '" 已经在队列中!');
					this.cancelUpload(file.id);
					this.queueData.filesCancelled++;
					return false;
					/*
					var replaceQueueItem = confirm('该文件名为 "' + file.name + '" 已经在队列中.\n您想在队列中，以取代现有的文件吗？');
					if (!replaceQueueItem) {
						this.cancelUpload(file.id);
						this.queueData.filesCancelled++;
						return false;
				...
				switch(errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							this.queueData.errorMsg = '最大支持选择文件个数为： ' + settings.queueSizeLimit  + '.';
						} else {
							this.queueData.errorMsg = '\n该选择的文件数超过了队列的大小限制 (' + settings.queueSizeLimit + ').';
						}
						break;
		...
		onUploadProgress : function(file, fileBytesLoaded, fileTotalBytes) {
			...
			if ($.inArray('onUploadProgress', settings.overrideEvents) < 0) {
				if (settings.progressData == 'percentage') {
					if(percentage == 100){
						$('#' + file.id).find('.data').html(' - '+ '100%，正在解析...');
					}else{
			...
			if (settings.checkExisting) {
				$.ajax({
					...
					success : function(data) {
						if (data == 1) {
							var overwrite = confirm('文件的名称"' + file.name + '" 已存在服务器.\n您是否进行替换?');
							if (!overwrite) {
								this.cancelUpload(file.id);
								$('#' + file.id).remove();
